...
export default async function handler(
  ...
  const rateLimit = checkRateLimit(`certificate-entrada:${clientIP}`, 20, 60000);
  ...
  try {
    console.log('Generando certificado PDF de ENTRADA, URL:', `${CERTIFICATE_ENTRADA_API_URL}/generate-certificate`);
    ...
    // Hacer la solicitud al servidor de certificados de entrada
    const response = await fetch(`${CERTIFICATE_ENTRADA_API_URL}/generate-certificate`, {
      method: 'POST',
      headers: {
      ...
      if (fetchError.name === 'AbortError') {
        throw new Error('Timeout al generar PDF de entrada (más de 30 segundos)');
      }
      throw fetchError;
    });
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        success: false,
        error: errorData.error || errorData.message || errorData.detail || 'Error al generar certificado de entrada',
      });
    }
    ...
    if (contentType.includes('application/pdf')) {
      ...
      return res.status(200).json({
        ...
        message: data.message || 'Boleta de entrada generada correctamente',
        error: data.error,
      });
    }
  } catch (error) {
    console.error('Error en proxy de certificados de entrada:', error);
    res.setHeader('Access-Control-Allow-Origin', '*');
    return res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Error desconocido al generar certificado de entrada',
    });
  }
}
